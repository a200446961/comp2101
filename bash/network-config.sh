...
#         host names, etc.
h=$(hostname)
l=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
lan=$(getent hosts $l)
ex=$(curl -s icanhazip.com)
ext=$(getent hosts $ex | awk '{print $2}')
...
ra=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
rh=$(getent hosts $ra | awk '{print $2}')
